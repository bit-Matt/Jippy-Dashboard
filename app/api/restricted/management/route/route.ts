...
export async function GET() {
  const data = await management.getAllRoutes();
  return oneOf(data).match(
    s => ResponseComposer.compose(StatusCodes.Status200Ok)
      .setBody(s)
      .orchestrate(),
    e => ExceptionResponseComposer.compose(StatusCodes.Status500InternalServerError, [{ message: e.value }])
      .orchestrate(),
  );
}
