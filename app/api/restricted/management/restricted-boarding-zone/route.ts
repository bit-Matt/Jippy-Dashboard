...
export async function GET() {
  ...
  try {
    const allZones = await unwrap(rbz.getAllRestrictedBoardingZones(false));
...
export async function POST(req: NextRequest) {
  ...
  const result = await rbz.createRestrictedBoardingZone(data, currentSession.user!.id);
  return oneOf(result).match(
    s => {
      void logActivity({
        ...
        action: "restricted_boarding_zone_created",
        summary: `Created restricted boarding zone ${s.name}`,
        routePath: "/api/restricted/management/restricted-boarding-zone",
        httpMethod: "POST",
