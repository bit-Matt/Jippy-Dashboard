...
export async function PATCH(
  request: NextRequest,
  { params }: RouteContext<"/api/restricted/management/restricted-boarding-zone/[id]">,
) {
  ...
  const result = await rbz.updateRestrictedBoardingZone(id, data);
  return oneOf(result).match(
    success => {
      void logActivity({
        ...
        action: "restricted_boarding_zone_updated",
        summary: `Updated restricted boarding zone ${id}`,
        routePath: `/api/restricted/management/restricted-boarding-zone/${id}`,
        httpMethod: "PATCH",
...
export async function DELETE(
  ...
  if (!utils.isUuid(id)) {
    return ApiResponseBuilder.createError(StatusCodes.Status404NotFound, [{ message: "Invalid restricted boarding zone ID" }])
      .build();
  }

  try {
    const isModifiable = await unwrap(rbz.isRestrictedBoardingZoneModifiable(id));
    ...
    const result = await rbz.removeRestrictedBoardingZone(id);
    return oneOf(result).match(
      () => {
        void logActivity({
          actorUserId: currentSession.user!.id,
          actorRole: currentSession.user!.role,
          category: "write_operation",
          action: "restricted_boarding_zone_deleted",
          summary: `Deleted restricted boarding zone ${id}`,
          routePath: `/api/restricted/management/restricted-boarding-zone/${id}`,
          httpMethod: "DELETE",
          statusCode: StatusCodes.Status200Ok,
          entityType: "restricted_boarding_zone",
